...
cd $FF_HOME
HIGH_FOLDER=$FF_HOME/basic-test-${GPUS}_gpus
...
do
  ...
  echo "python fsdp.py --world_size ${GPU} --epochs ${EPOCHS}"
  python fsdp.py --world_size $GPU --epochs $EPOCHS 2>&1 | tee output.txt
