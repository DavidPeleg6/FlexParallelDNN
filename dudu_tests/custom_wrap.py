...
# This code is almost identical to the auto_wrap.py file. the only change is that the recursive wrap passes the name of
# the module in question in addition to the original values
# TODO move all these changes into auto_wrap.py once youll be able to recompile fairscale locally in your pod
def row_parallel_wrap_policy(
    module: nn.Module,
    recurse: bool,
    ...
    **kwargs: Any,
    ...
    """Wrap a module with a parallel linear layer
    ...
    Args:
       ...
       strategy (dict):
            a dictionary with a list of modules and their corresponding strategies
       module_name (str):
            the name of the module to be wrapped
       min_num_params (int):
    ...
    # TODO change this to get a type filter as well and the isinstance will be compared to that
    should_wrap = isinstance(module, cast(type, nn.modules.linear.Linear)) \
                  and module_name in strategy.keys() \
                  and unwrapped_params >= min_num_params \
                  and strategy[module_name].row_linear
    if recurse:
        # We should recurse if the module is big enough but not in force_leaf_modules list.
        return should_wrap and not isinstance(module, tuple(force_leaf_modules))
    else:
...
# the module in question in addition to the original values
# TODO move all these changes into auto_wrap.py once youll be able to recompile fairscale locally in your pod
def column_parallel_wrap_policy(
    module: nn.Module,
    recurse: bool,
    unwrapped_params: int,
    # These are customizable for this default policy function.
    ...
    **kwargs: Any,
    ...
    should_wrap = isinstance(module, cast(type, nn.modules.linear.Linear)) \
                  and module_name in strategy.keys() \
                  and unwrapped_params >= min_num_params \
                  and strategy[module_name].column_linear
    if recurse:
        # We should recurse if the module is big enough but not in force_leaf_modules list.
        return should_wrap and not isinstance(module, tuple(force_leaf_modules))
    else:
...
# the module in question in addition to the original values
# TODO move all these changes into auto_wrap.py once youll be able to recompile fairscale locally in your pod
# TODO change this to data parallel wrap or combine it with the linear wrap
def data_parallel_wrap_policy(
    module: nn.Module,
    recurse: bool,
    unwrapped_params: int,
    # These are customizable for this default policy function.
    ...
    **kwargs: Any,
    ...
    """Decide whether to wrap a module with data parallel.
       Return if a module should be wrapped during :func:`auto_wrap`. This function is an extension of default policy
    ...
    should_wrap = module_name in strategy.keys() \
                  and unwrapped_params >= min_num_params \
                  and (strategy[module_name].gather_input or strategy[module_name].split_output or
                       strategy[module_name].data_parallel_input)
    if recurse:
        # We should recurse if the module is big enough but not in force_leaf_modules list.
        return should_wrap and not isinstance(module, tuple(force_leaf_modules))
    else:
    ...
    **kwargs: Any,
    ...
    Args:
       ...
       min_num_params (int):
           Customizable policy input. It controls the size threshold
           on how big should a module be to be considered wrapped.
       type_filter (nn.Module): wrapping of the layer should occur only in case the type of layer matches the filter
                                defaults to None.
       force_leaf_modules (Set[Type[nn.Module]]): set of module types to
           keep as leaves, i.e., their children will never be wrapped.
       exclude_wrap_modules (Set[Type[nn.Module]]):
    ...
    type_match = True if type_filter is None or isinstance(module, cast(type, type_filter)) else False
    should_wrap = is_large and type_match
    if recurse:
        # We should recurse if the module is big enough but not in force_leaf_modules list.
        return should_wrap and not isinstance(module, tuple(force_leaf_modules))
    else:
...
def custom_wrap(module: nn.Module, module_name: str, **wrap_overrides: Any) -> nn.Module:
    """
    ...
    Args:
        ...
        module_name: the name of the current layer wrapped
...
class ConfigAutoWrap:
    ...
    @staticmethod
    def custom_recursive_wrap(module: nn.Module, auto_wrap_policy: Optional[Callable], name: str,
        ...
        Args:
            ...
            name:
                the name of the module to be wrapped. will be used as a prefix
            auto_wrap_policy (Callable, Optional):
        ...
        for layer_name, child in module.named_children():
            # set the prefix in accordance with the recursion
            prefix = name + ('.' if name else '') + layer_name
            # print(prefix)
            wrapped_child, num_wrapped_params = ConfigAutoWrap.custom_recursive_wrap(
                module=child, auto_wrap_policy=auto_wrap_policy, name=prefix, **kwargs
            )
            setattr(module, layer_name, wrapped_child)
            # Keep track of how many parameters have been wrapped
            total_wrapped_params += num_wrapped_params
        # decide if we need to wrap the current module,
        # since the left over parameters exceed the number of params to wrap
        remainder = num_params - total_wrapped_params
        if auto_wrap_policy(module=module, recurse=False, unwrapped_params=remainder, module_name=name, **kwargs):
