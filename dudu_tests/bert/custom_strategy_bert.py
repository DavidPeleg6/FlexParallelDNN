...
def init_processes(train_args):
    ...
    # # TODO recompile pytorch from source to support mpi for pipeline parallelism
...
def main_loop(rank, train_args):
    ...
    for i in range(train_args.budget):
        ...
        if avg_ips > best_ips:
            best_strategy = train_args.strategy.copy()
            best_ips = avg_ips
        # todo add second best strategy to see who came close to the baseline. then you can pinpoint how far you were from the baseline
        if dist.get_rank() == 0:
            print(f'iteration: {i}, '
...
def train_and_test(rank, train_args):
    ...
    # # TODO move the vanilla strategy creation somewhere else like the optimizer would by default create a new strategy if none was given
    ...
    optimizer = OSS(params=model.parameters(), optim=optim.Adam, **optim_kwargs)
    # todo uncomment this if you turn back to fsdp
