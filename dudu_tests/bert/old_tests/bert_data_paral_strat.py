...
from dudu_tests.strategy_handler import create_vanilla_strategy, import_strategy
...
def train_and_test(rank, train_args):
    ...
    strategy = strategy_from_file(train_args.strategy)
    if strategy is None:
        strategy = create_vanilla_strategy(model, 'strategies/bert_vanilla.txt', min_num_params=int(1e4))
    print(strategy)
    ...
    with custom_wrap.enable_wrap(**data_params):
        ...
        #     with open('strategies/pytorch_strategy.txt', 'w') as file:
        #         # js = json.dumps({name: GatherSplit(module.gather_input, module.split_output) for name, module in
        #         #                  model.named_modules() if isinstance(module, DataParallelLayer)}, indent=4)
        #         js = json.dumps({name: GatherSplit(False, False) for name, module in
        #                          model.named_modules()}, indent=4)
        #         file.write(js)
        ...
        my_auto_wrap_policy = functools.partial(custom_wrap.strategy_wrap_policy, strategy=strategy)
    ...
    with enable_wrap(**fsdp_params):
        ...
        # my_auto_wrap_policy = functools.partial(default_auto_wrap_policy, min_num_params=int(0))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--strategy", action="store", default='', type=str)
