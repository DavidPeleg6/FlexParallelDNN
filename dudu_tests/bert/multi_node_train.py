...
def train_and_test(rank, train_args):
    ...
    # FSDP custom wrap
    fsdp_params = dict(wrapper_cls=FSDP, mixed_precision=False, flatten_parameters=False,
                       verbose=train_args.print_params)
    ...
    # FSDP vanilla
    model = FSDP(model, verbose=train_args.print_params, flatten_parameters=False, mixed_precision=False)
    model = model.to(rank)
    ...
    # # todo delete this
    # if rank == 0:
    #     print(model.local_state_dict()['encoders.19.ff.linear1.weight'].shape)
...
if __name__ == "__main__":
    ...
    # # ======================== BERT LARGE TEST (more than 12GB parameters)
    ...
    # # todo use this to show that fsdp allows for larger networks than ddp
    # parser.add_argument("--encoder_layers", type=int, default=57)
    ...
    parser.add_argument('--print_params', dest='print_params', default=True, action='store_true')
